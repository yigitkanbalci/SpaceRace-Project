...
void LCD_Reset(void)
{
    ...
    nanoWait(100000000); // Wait
    lcddev.reset(0);      // De-assert reset
    nanoWait(50000000);  // Wait
}
...
void LCD_Init(void (*reset)(int), void (*select)(int), void (*reg_select)(int))
{
    ...
    nanoWait(120000000); // Wait 120 ms
...
{
    TempPicturePtr(tmp,45,45); // Create a temporary 29x29 image.
    pic_subset(tmp, &space, x-tmp->width/2, y-tmp->height/2); // Copy the space
...
void update_ship(int x, int y)
{
    TempPicturePtr(tmp,45,45); // Create a temporary 29x29 image.
    pic_subset(tmp, &space, x-tmp->width/2, y-tmp->height/2); // Copy the space
    pic_overlay(tmp, 11,11, &spaceship, 0xffff); // Overlay the ship
    LCD_DrawPicture(x-tmp->width/2,y-tmp->height/2, tmp); // Draw
}

void update_ast(int x, int y, int overlay)
{
    TempPicturePtr(tmp,25,25); // Create a temporary 29x29 image.
    pic_subset(tmp, &space, x-tmp->width/2, y-tmp->height/2); // Copy the space
    pic_overlay(tmp, overlay, overlay, &asteroidpic, 0xffff); // Overlay the asteroid
    LCD_DrawPicture(x-tmp->width/2,y-tmp->height/2, tmp); // Draw
}
...
{
    TempPicturePtr(tmp,35,35); // Create a temporary 29x29 image.
    pic_subset(tmp, &space, x-tmp->width/2, y-tmp->height/2); // Copy the space
    pic_overlay(tmp, overlay, overlay, &asteroidpic, 0xffff); // Overlay the asteroid
    LCD_DrawPicture(x-tmp->width/2,y-tmp->height/2, tmp); // Draw
}
